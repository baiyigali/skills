...
- **同时询问一次交付目录**（只问一次，不反复确认）：本次报告的 .md 与封面 .png 要保存到哪里。若用户没有指定，保存到当前工作区。
- **文件命名约定**：默认命名 `竞品分析报告_NN_主题.md`（NN 为两位递增序号，主题取自 TOPIC，不含空格）。若交付目录中已存在同系列文件，取现有最大序号 +1 作为本次 NN；封面 png 与 md 同名仅扩展名不同。
- 判断输入类型并确定分析模式：
...
- 输出为单个 .md 文件（禁止 doc/docx），按第 1 步约定的文件名保存到交付目录（目录不存在时先创建）；封面 .png 同名保存在同一目录。
- 仅输出完整报告成品，不在正文中暴露中间检索、整理、思考过程。

### 5. 生成封面图片

1. 先根据文章内容拟 2 个不同版本的专业长英文图片提示词（电影镜头质感、高画质、写实主义；人物用中国人/亚洲人形象；**画面中不含任何文字**）。提示词不输出到正文。
2. 使用可用的图像生成工具按提示词生成封面图，保存为 .png，文件名与文章文件名同名，放在文章同级目录。
3. 如宿主环境支持，将图片上传云端取直链 URL；如生成图片超过上传大小限制，先压缩（如 macOS `sips -s format jpeg -s formatOptions 82 -Z 1400 <原图> --out /tmp/cover.jpg`）再上传。
4. 在文章开头标题下方以 `![封面](云端URL)` 的 markdown 形式插入封面。无云端上传能力时改为引用本地相对路径。

### 6. 收尾

- 文末在参考资料之后另起一行附适合公众号的话题标签。
- 话题标签之后另起一行附作者署名（软性推广，一行即可，不得再加二维码或诱导关注文案）：

  > \*本报告由「程序员白大力」的竞品分析工作流生成，欢迎关注公众号获取系列更新。\*

- 用可用的文件呈现方式将 .md 报告和封面图一并交付给用户。

## Resources

- `references/report-template.md` — 完整的固定报告目录、三阶段执行细则、写作硬性要求、输出格式与封面要求。执行本技能任何阶段前先完整阅读。
