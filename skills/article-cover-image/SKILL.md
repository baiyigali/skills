...
- **尺寸与额度（能省则省）**出图工具对尺寸有硬下限：**宽高都必须 ≥ 256**，传 `384x164` 这类更小的尺寸会直接报错（invalid parameter size）。2.35:1 下的可用档位：

  | 档位 | 尺寸 | 用途 |
  |---|---|---|
  | 省额度档（**默认**） | `608x259` | 日常封面、多图文里的**次条**（列表只显示小缩略图） |
  | 标准档 | `1024x448` | 单篇推送、或多图文的**首条大图** |
  | 高清档 | `1536x640` | 仅海报、置顶大图等确有需要的场景 |

  `quality` 默认 `low`；只有首条大图或确有画质要求时才用 `medium`/`high`。
- **多图文场景优先省张数**：一次多图文推送里，只有首条的封面是大图展示，其余各条只显示小缩略图。因此批量出图时，首条用标准档、**其余一律用省额度档**；若调用方允许共用封面，也可以只出 1 张、其余复用，这是最省的做法（8 张 → 1 张）。
- 保存为**真实 PNG**（不是改后缀）：下载后转格式，并用 `file` 确认类型
- 文件名与 md **同名**，放同级目录
- **交付目录卫生（硬性）**：出图工具的原始产物先落在临时目录（工作区 generated-images），处理完再写入同名 .png。**未裁切的原始图、备选版图、v2 试验图一律不得留在交付目录**——它们会被 git 一起提交，白白撑大仓库。交付目录里只允许出现 `<主名>.md`、`<主名>.png`、`<主名>-prompts.md`、`.cover-log.md`
